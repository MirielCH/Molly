...
COMMAND_CLAIM = re.compile(r"(?:\bclaim\b|\bcl\b|\buse\b\s+(\bvalentine\b\s+\bmuffin\b|\blootbox\b\s+\bcake\b))")
...
COMMAND_MINIEVENT_BUY = re.compile(r"(?:\bme\b|\bminievent\b)\s+\bbuy\b")
...
COMMAND_RAID = re.compile(r"(?:\braid\b|(?:\buse\b|\bconsume\b|\bconsoom\b).+(\bchocolate\b\s+\bcarrot\b|\bworker\b\s+\bpopsicle\b))")
COMMAND_REQUEST = re.compile(r"(?:\bre\b|\brequest\b)")
COMMAND_SHOP = re.compile(r"\bshop\b")
COMMAND_TEAMRAID = re.compile(r"\bteamraid\b")
COMMAND_USE_BOOST = re.compile(r"(?:\buse\b|\bconsume\b|\bconsoom\b).+(?:\bpopper\b|\bboost\b|\bworger\b|\beaster\b)")
...
COMMAND_USE_ENERGY_TANK = re.compile(r"(?:\buse\b|\bconsume\b|\bconsoom\b)\s+\benergy\b\s+\btank\b")
COMMAND_USE_ERNGY_CLOVER = re.compile(r"(?:\buse\b|\bconsume\b|\bconsoom\b)\s+\berngy\b\s+\bclover\b")
